...
def parallelise(func, args_list, max_workers=None, **kwargs_list):
    ...
    for key, value in kwargs_list.items():
        ...
        elif len(value) == 0:  # Check for empty list
            replicated_kwargs[key] = [[] for _ in range(len(args_list))]  # Create a list of empty lists
        else:
            replicated_kwargs[key] = value

    # Combine args and kwargs into a list of tuples
    combined_args = [(args,) + tuple(replicated_kwargs[key][i] for key in replicated_kwargs)
