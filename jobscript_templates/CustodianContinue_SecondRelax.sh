...
jobs = [VaspJob(sys.argv[1:], output_file=output_filename, suffix = ".relax_2", final=False,
        ...
        VaspJob(sys.argv[1:], output_file=output_filename, suffix = "",
                settings_override = [{"dict": "INCAR", "action": {"_set": {"NSW": 0, "LAECHG": True, "LCHARGE": True, "NELM": 240, "EDIFF": 1E-5}}},
                                     {"file": "CONTCAR", "action": {"_file_copy": {"dest": "POSCAR"}}}], copy_magmom=True)]
...
then
        ...
        python custodian_vasp.py srun -n $ncpu --exclusive --mem-per-cpu=0 -m block:block,Pack vasp_std &> vasp.log
fi
...
srun --export=ALL -N 2 -n $ncpu Chargemol_09_26_2017_linux_parallel
