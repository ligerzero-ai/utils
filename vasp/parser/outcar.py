...
class Outcar(object):
    ...
    def from_file(self, filename="OUTCAR"):
        ...
        self.parse_dict["execution_datetime"] = run_datetime
    ...
    def get_datetime(self, filename="OUTCAR", lines=None):
        if lines is None:
            with open(filename, "r") as f:
                lines = f.readlines()

        # Regular expression to extract the date and time
        pattern = r"executed on.*date (\d{4}\.\d{2}\.\d{2})\s+(\d{2}:\d{2}:\d{2})"
